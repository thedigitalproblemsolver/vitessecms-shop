...
class OrderController extends AbstractController implements RepositoriesInterface
{
    ...
    public function saveAndPayAction(): void
    {
        if ($this->user->isLoggedIn()):
            ...
            $country = $this->countryRepository->getById($shiptoAddresses->country);
            $shiptoAddresses->set('countryName', $country->getRaw('name'));
            ...
            if ($this->session->has('discountId')) :
                ...
                if ($discount !== null) :
                    ...
                    if (DiscountEnum::TARGET_ORDER === $discount->getTarget()) :
                        ...
                        $order->setTotalDiscountDisplay(
                            PriceUtil::formatDisplay(
                                (float)$order->_('totalDiscount')
                            )
                        );
                        $order->setTotal(
                            DiscountHelper::calculateTotal(
                                (float)$order->_('total')
                            )
                        );
                        $order->setTotalDisplay(
                            PriceUtil::formatDisplay(
                                (float)$order->_('total')
                            )
                        );
    ...
    public function viewOrderAction(string $id): void
    {
        $displayError = true;
        if ($this->user->isLoggedIn()) {
